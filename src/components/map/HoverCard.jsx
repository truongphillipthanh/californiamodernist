...
export default function HoverCard({ project, position, onMouseEnter, onMouseLeave }) {
  // TASK-040: Don't render if no valid project
  // Note: position is optional - if not provided, renders inline (for sidebar photo view)
  if (!project) return null;

  // If position is expected (map context) but missing, don't render
  // This check is for when HoverCard is used as map overlay
  const isMapOverlay = position !== undefined;
  if (isMapOverlay && !position) return null;

  const status = STATUS_COLORS[project.status] || STATUS_COLORS.active;

  // Sidebar/inline mode (no position)
  if (!isMapOverlay) {
    return (
      <div className="w-full bg-white overflow-hidden">
        ...
        <div className="p-4">
          ...
          <p className="text-sm text-stone-500 mt-0.5">
            {project.address || project.parcel?.address}
          </p>
  ...
  // Map overlay mode (with position)
  return (
    <div
